...
server.registerTool(
  "send_device_commands",
  {
    ...
    description:
      "Writes to the hub. One-shot device commands (on/off, brightness, position, setpoint) for one or many devices in one call; take characteristic refs from read_services. All commands are checked against the live contract first: if any is invalid, nothing is sent and each invalid item is listed. They then run in order; each is decided by a read right before its write and reports applied, already_desired, uncertain, conflict, rejected or not_sent, previous_value and change_ref for get_native_change. uncertain is not proof that nothing happened; a repeat holds a value whose earlier send got no answer, and that item's next resends it. Commands are physical actions, not undone; restore_native_change restores only restore_supported=true items. For configuration changes use prepare_native_change.",
    inputSchema: {
      home_ref: z.string().min(1).describe("Exact home_ref from list_homes."),
      commands: z
        .array(

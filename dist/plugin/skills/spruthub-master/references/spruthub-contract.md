...
- Наблюдатель сохраняет повторные события, которые фактически дошли до WebSocket, но этот поток не является полным журналом физических или радиоотчётов. На проверенном command path повторная подтверждённая команда с тем же значением не породила второе characteristic event и новую проверку BLOCK; это не устанавливает поведение radio path датчика. [Обезличенное наблюдение границы command/event](https://github.com/xor777/sprut-agent/blob/04ef59d629f01d4c31adcd878263d0b72a351b00/research/protocol/2026-09-11-native-event-boundary.md).

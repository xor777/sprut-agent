...
- `logic_source_create` принимает точный service ref, полный source и явные
  ...
  `logic_mapping_reason=logic_type_not_mapped_at_create`. Если delete такой
  записи уже отправила прежняя версия, отсутствие сценария даёт `restored` с
  `logic_assignments_may_remain=true`: перед delete проверялись назначения не
  доказанного type, и назначения его собственного type могли остаться на
  устройствах. Скажи об этом пользователю. Собственный маркер в source позволяет найти

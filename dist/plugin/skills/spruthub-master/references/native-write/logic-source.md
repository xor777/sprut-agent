...
- `logic_source_create` принимает точный service ref, полный source и явные
  ...
  созданный LOGIC только включённым: видны ли назначения выключенного LOGIC
  в `logic.list`, на хабе не проверяли, и пустой обход этого не доказывает.
  Если ничего не найдено, выключенный LOGIC restore не удаляет и отвечает
  `logic_off_assignments_unverified` без `next`. Включённый LOGIC работает на
  всех устройствах, где он назначен, поэтому включать его ради отмены можно
  только с согласия владельца; скажи это пользователю. Если владелец
  согласен, включи LOGIC через [`scenario_active`](scenario-active.md) и
  повтори restore. Если restore тогда найдёт назначения, он ничего не удалит
  и LOGIC останется включённым; выключает его restore того же
  `scenario_active` change. Иначе владелец удаляет LOGIC в приложении
  SprutHub. Если delete записи уже отправила прежняя версия,

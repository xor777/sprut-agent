...
- `logic_source_create` принимает точный service ref, полный source и явные
  ...
  Restore обходит назначения его type по дому и цели BLOCK, включён LOGIC
  или выключен: найденное блокирует удаление (`logic_assignments_present` со
  списком сервисов). SprutHub 3.0.0 показывает в `logic.list` назначение,
  сделанное при включённом LOGIC, и после выключения, а выключенный LOGIC
  назначить нельзя, поэтому обход видит и назначения выключенного. Если
  ничего не найдено, restore удаляет созданный LOGIC и выключенным, не
  включая его. Если delete записи уже отправила прежняя версия,

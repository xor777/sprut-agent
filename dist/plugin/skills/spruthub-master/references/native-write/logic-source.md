...
- `logic_source_create` принимает точный service ref, полный source и явные
  ...
  create. Type, появившийся позже, не сопоставляется: он может принадлежать
  другому LOGIC. При `missing` (новых type нет: так SprutHub 3.0.0 показал
  выключенный LOGIC, а `logic.types` читается по сервису) и `ambiguous`

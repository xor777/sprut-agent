# История и восстановление native changes

Используй эту карточку вместе с карточкой выбранной операции, когда исход записи неизвестен, нужен повтор после restart, обнаружена ручная правка или требуется restore. До обычного prepare достаточно свежего `get_native_change_contract` и карточки самой операции.

Общий публичный write-path сначала раскрывает версионный контракт через
`get_native_change_contract`, затем разделяет `prepare_native_change`,
`apply_native_change`, `get_native_change` и `restore_native_change`.
Статические контракты `room_create` и `virtual_light_group` читаются по одной
`operation` без угадывания цели. Явно переданный `target_ref` для них должен быть
точной ссылкой настроенного дома; прочие зависимые от сущности контракты требуют
подходящую home-qualified ссылку. Это чтение описывает возможность механизма, но
не проверяет выбранные комнаты, лампы или готовность конкретного изменения.
`list_native_changes` даёт небольшую страницу истории по дому и точной entity ref,
включая старые automation changes. Возвращённый `next` сохраняет дом, фильтр и
размер страницы; его выполняют до явного `null`. Cursor другой области отклоняется,
а сохранённая граница порядка продолжает работать, если ограничивающую запись
прочитали, переместили или удалили. Страницы читают текущий порядок журнала, а не
атомарный снимок: новое или обновившееся изменение перед cursor не появляется в
его продолжении. `recorded_status` —
сохранённый исход на `updated_at`, а не текущая проверка; для неё используется
адресный `next` самой записи. У `block_action_pause` отдельный `effect_status` вычисляется из
сохранённого абсолютного срока и не выдаёт `applied` за ещё активное исключение.
Для BLOCK история индексирует canonical refs
сценария и участвующих accessory/service/characteristic до и после update;
старые unqualified refs привязываются только к fingerprint текущего журнала.
Это поиск, а не разрешение на запись.

Сохранённое намерение различает apply и restore, а ACK относится только к
текущей попытке. Ответ хаба с ошибкой на саму запись (любой JSON-RPC код,
включая `-32601` «метод не найден») — явный отказ, а не неизвестный исход:
вызов возвращает ошибку с `rejection` (код и текст хаба как данные) и
`hub_effect=not_applied`, apply оставляет change в `not_applied`, restore — в
прежнем `applied`, а `write_intent.rejection` сохраняет причину. Сверять
такой отказ не нужно; `uncertain` остаётся только для timeout, разрыва
соединения и нераспознанного ответа после отправки. Исключение —
многошаговый `virtual_light_group`: если хаб уже выполнил создание accessory
или часть шагов restore, отказ следующего шага даёт `hub_effect=partial`,
`change` с записанным состоянием (как у следующего get:
`uncertain`, `verification.result=owned_partial_group_observed`) и `next`:
после apply — `restore_native_change`, который удаляет созданное, после
restore — `get_native_change`. После такого отказа повторный apply
продолжает недостающие шаги, а повторный restore — удаление. `verification.fresh=false` означает отсутствие нового
readback: прошлый outcome и последнее наблюдение могут быть показаны, но не
считаются текущей проверкой. Restore ещё не применённого `block_create`,
`block_data_update`, `block_action_pause`, `logic_source_create` или
`logic_source_update` без proven apply и без сохранённого conflict даёт
`not_owned`/`change_was_not_applied` без записи и не запрещает последующий
apply того же черновика. Неотправленный `block_action_pause` не ищет чужой
controller и не объявляет conflict: неизвестная отправка, доказанный эффект
и ручная правка после apply остаются прежними исходами.
Сохранённый conflict `block_action_pause` restore не переводит в
`not_owned` и не открывает apply после возврата baseline: get, restore и
повторный apply остаются `conflict`, следующий шаг — новый prepare.
Журнал 0.1.41, застрявший в `conflict/pause_controller_changed` без proven
apply, этим restore не лечится.
`not_owned` после отклонённой отправки не становится
`applied` и не даёт restore по позднему совпадению с requested: чужой source
остаётся чужим, пока агент явно не применит тот же черновик на текущем
baseline. Для BLOCK/LOGIC conflict без сохранённого
applied snapshot совпадение с requested не восстанавливает право записи:
get, restore и повторный apply остаются `conflict`, `restore_supported=false`,
а следующий шаг — новый prepare на текущем baseline. Подтверждённый apply
того же change больше не отправляется: ручной откат к baseline остаётся
conflict, не перезаписывается повторным apply, и новое намерение идёт через
свежий prepare. Завершённый restore native-value закрыт: get может обновить
текущее значение или ошибку чтения, но не превращает change снова в applied
и не даёт права записи после совпадения с requested. Restore, заблокированный текущими назначениями LOGIC, не
предлагает новый prepare: сначала нужна отдельная разрешённая работа с этими
назначениями. Явный restore по доказанному applied snapshot не меняется.
Сохранённое через get/apply/restore отсутствие proven BLOCK/LOGIC create по
записанному index прекращает delete этого change; совпадение копии право не
возвращает. Общие чтения дома этот журнал не ведут.

Локальная история привязана к URL и serial хаба через SHA-256 и не содержит
token. По умолчанию она хранится вне Git; `SPRUT_AGENT_STATE_DIR` задаёт
изолированный каталог, а пустое значение считается незаданным. Один MCP-процесс сериализует apply/restore, но
межпроцессной блокировки нет. Preview не резервирует состояние до apply, а в
наблюдённом API нет compare-and-set или conditional delete.

Options других владельцев и форм, pairing, backup, GLOBAL и иные виды
произвольного кода этим контрактом не поддерживаются.
Первый login настраивается локальным credential-файлом и не является native
change. Не подменяй отсутствующие операции raw RPC или `write: true` в данных.

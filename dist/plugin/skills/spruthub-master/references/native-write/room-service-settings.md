...
- Имя обрезается по краям, пустое отклоняется. Имя комнаты длиннее 32
  символов или с эмодзи `room_name` не готовит (`name_too_long`,
  `name_characters_unsupported`), см. [`room_create`](room-create.md). Для
  имени сервиса такой предел не проверяли, и он не применяется. Если такое же точное имя уже
...
- `room.update` с новым именем проверен на хабе 3.0.0 (переименование и
  возврат); `service.update` известен только по схеме протокола. Сохранил ли
  хаб значение, показывает readback, а не ACK.

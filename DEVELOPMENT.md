...
печатается как `PASS* (unverified: room.update)`. `summary.json` сводит повторы: k/n успехов, медианы MCP-вызовов,
MCP-байтов, запросов к хабу, байтов его ответов и токенов по задаче и дому и
отношение house/apartment. Для сравнения между Claude и Codex берутся

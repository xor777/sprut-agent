...
id). Оценка детерминированная: по записям и итоговому состоянию симулятора,
по выполнению созданных BLOCK-правил и по смыслу ответа (правила чтения
ответа — в начале `research/eval-agent-cases.mjs`). Прогон вызывает модель,
...
`simulator_gap`, а не как ошибку агента. Созданное правило грейдер
...
провала, MCP-вызовы и их байты, запросы к хабу и байты его ответов, все
вызовы, токены, время, версия CLI и фактическая модель, идентичность плагина
...
успешным прогонам; без успеха на одном из домов его нет. Для сравнения между Claude и Codex берутся

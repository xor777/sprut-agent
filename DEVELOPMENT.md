...
провала, MCP-вызовы и их байты, все вызовы, токены, время, версия CLI и
фактическая модель, идентичность плагина по `server_sha256`, методы
симулятора с уровнем доказанности `observed`/`schema_only`/`guess`, сработавшие
сбои), транскрипт и запросы к симулятору. `summary.json` сводит повторы:
k/n успехов, медианы MCP-вызовов, MCP-байтов и токенов по задаче и дому и
отношение house/apartment. Для сравнения между Claude и Codex берутся
MCP-вызовы и MCP-байты: собственные инструменты агентов разные.

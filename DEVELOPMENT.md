...
исполняет по шагам BLOCK; форму, которую он не моделирует (удержание
условия, повтор ветки, задержка не в режиме RESET, код), он не угадывает:
прогон проваливается как `grader_unsupported`. Задачи с `pending` (недостающая

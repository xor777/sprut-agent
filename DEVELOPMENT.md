...
Удаление идёт через `restore_native_change`. Объект со своим префиксом,
который restore оставил, скрипт затем удаляет напрямую клиентом продукта,
если продукт доказуемо его создал и он не может сработать: сценарий с
маркером продукта и `active=false`, пустая комната из `room_create` журнала
прогона. Каждый такой объект — провал прогона; остальные объекты с префиксом
только перечисляются. `-- --sweep-only <префикс> --state-dir <каталог>`
повторяет эту уборку для прошлого прогона; каталог журнала печатается в конце
прогона, без него комнаты только перечисляются.

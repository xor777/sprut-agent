...
- `logic_source_create` принимает точный service ref, полный source и явные
  ...
  `logic.types` читается по сервису, поэтому без однозначного type назначение
  на другом сервисе исключить нельзя. При `ambiguous` любой из новых type
  может быть этим LOGIC, при `missing` (так SprutHub 3.0.0 показал выключенный
  LOGIC) type не виден на выбранном сервисе, но может быть назначен на другом.
  В обоих случаях `restore_supported=false`, а restore ничего не удаляет и
  отвечает ошибкой `ambiguous_logic_type` (с `candidate_logic_types`) или
  `logic_type_not_visible` со `scenario_ref`. Это не успех отмены: LOGIC
  остаётся на хабе. Скажи пользователю, что его можно удалить в приложении
  SprutHub, убедившись, что ни одно устройство его не использует; restore
  повторяет проверку и удалит LOGIC, когда mapping станет `mapped`, а
  назначений не будет. Собственный маркер в source позволяет найти

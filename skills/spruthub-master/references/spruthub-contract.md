...
- `get_scenario_sdk` читает полный текущий native `scenario.sdk` выбранного хаба по явному запросу и возвращает размер, SHA-256 и свежесть. Это протокольный источник API исполняемого на хабе кода, а не Node/browser SDK; старый snapshot/frontend schema остаётся только кандидатом до проверки.

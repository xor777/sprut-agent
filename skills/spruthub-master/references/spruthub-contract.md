...
- `virtual_light_group` требует не менее двух `Lightbulb` service с общими
  `On` и `Brightness`, создаёт один виртуальный `Lightbulb` и связывает только
  эти явно выбранные характеристики в режиме `LastValue`. Этот режим сохраняет
  ...
  Поэтому apply значения группы читает каждого участника и возвращает
  `uncertain/group_members_not_converged`, если они не сошлись; общий scalar не
  доказывает результат. Restore удаляет только принадлежащие change links и

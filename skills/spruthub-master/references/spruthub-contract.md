...
- `logic_source_create` принимает точный service ref, полный source и явные
  ...
  удалять его до подтверждённого mapping нельзя: неизвестный type может иметь
  назначение на другом сервисе. Собственный маркер в description позволяет
  найти результат потерянного create без второй отправки. В журнале сохраняется
  точный source, но наружу change возвращает его SHA-256 и результат точного
  сравнения, а не повторяет сохранённый код.
- `logic_source_update` поддержан только для непрерывно наблюдаемого
  ...
  проверяет, что source изменён, а metadata/flags сохранены. Predefined source
  доступен для чтения и клонирования, но не изменяется. Restore update не
  затирает ручную правку. Restore create перед delete сканирует текущие

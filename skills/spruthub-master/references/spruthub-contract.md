...
- `read_services(home_ref, room_ref?, service_types?, max_bytes?, cursor?)` даёт ограниченную по фактическому UTF-8 размеру страницу native services с читаемыми текущими значениями. Каждый service сохраняет room и физический accessory; `service_types` сопоставляет точные native имена по OR, а `observed_service_types` показывает реально встреченные типы области. `false`, ноль, известный `null` и отсутствующее native значение различает `value_status`; enum и unit не выводятся из бытового названия. `scope_status` отделяет пустую область от `match_status: no_matches`. Продолжение доступно через `next`/cursor без обрезки отдельных значений; слишком крупный service явно оставляет точный `get_entity` next вместо подмены данных. Каждая страница читается заново и не является атомарным снимком дома.

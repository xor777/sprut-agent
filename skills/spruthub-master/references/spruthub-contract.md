...
- `logic_source_create` принимает точный service ref, полный source и явные
  ...
  `type=LOGIC`, читает source обратно как точный текст и устанавливает
  назначаемый type только по единственному новому значению свежего
  `logic.types`; `scenario.index`, имя и type не приравниваются. Собственный
  маркер в description позволяет найти результат потерянного create без второй
  отправки. В журнале сохраняется точный source, но наружу change возвращает его
  SHA-256 и результат точного сравнения, а не повторяет сохранённый код.
- `logic_source_update` поддержан только для непрерывно наблюдаемого
  пользовательского LOGIC и меняет source, сохраняя точные исходные
  metadata/flags. Predefined source доступен для чтения и клонирования, но не
  изменяется. Restore update не затирает ручную правку. Restore create перед
  delete сканирует текущие назначения наблюдённого native type по сервисам дома;

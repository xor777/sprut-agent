...
- Повтор apply одного change сохраняет исходный срок и не отправляет вторую
  обёртку. Новое поручение на тот же pointer заменяет одну проверенную собственную
  обёртку; поздний status/restore старого change становится `superseded` и не
  меняет хаб. Restore текущей паузы удаляет только неизменённый управляющий
...
- После срока обёртка инертна. Следующий подготовленный `block_data_update` того
  ...
  плановой записи и сохраняет актуальный дочерний узел. Read-only вызовы не
  чистят конфигурацию. Native source update может отменить уже запущенные delay
  прежней версии сценария; отдельного исполнения ровно в deadline нет.

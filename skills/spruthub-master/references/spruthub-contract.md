...
- `logic_source_update` поддержан только для непрерывно наблюдаемого
  ...
  проверяет, что source изменён и отдельно настраиваемые flags сохранены.
  Пересчитанный хабом из JS `description` сохраняется как наблюдённая metadata,
  а не считается ручным конфликтом. Predefined source доступен для чтения и
  клонирования, но не изменяется. Restore update не

...
- `get_scenario_sdk` читает текущий native `scenario.sdk` выбранного хаба по явному запросу. `sdk_complete=true` подтверждает полный возвращённый текст; при скрытии credential `sdk_complete=false`, а размер и SHA-256 относятся к возвращённому маркеру, не к недоступному исходнику. Это протокольный источник API исполняемого на хабе кода, а не Node/browser SDK; старый snapshot/frontend schema остаётся только кандидатом до проверки.
...
- `virtual_light_group` требует не менее двух `Lightbulb` service с общими
  ...
  При apply `characteristic_value` к группе, созданной продуктом и известной
  текущему журналу, инструмент читает каждого участника и возвращает
